...
Build system: CMake builds the core library, host tests, and optional host examples
...
Examples: `basic_status` implemented and integration-tested; remaining runnable examples remain Phase 4 work
Arduino adapter: not added yet
ESP-IDF adapter: not added yet
```

Implementation code and host tests exist for both complete-line entry points: `bsc_execute_line()` remains application-only, while `bsc_execute_line_with_builtins()` supports ordinary no-catalog help plus optional catalog-aware exact-path and final-token flat-topic routing. Tests cover exact registry identity and catalog validation at initialization, full-path precedence, `UNKNOWN_TOPIC` versus `UNKNOWN_COMMAND`, visibility, output failures, cleanup, recursion, and execution-callback isolation. Adapter and hardware validation remain future work.
