...
Implemented console help grammar is:
...
An optional borrowed catalog must share the console registry pointer and count exactly and is validated during initialization. With a catalog, the complete exact visible descriptor path wins; only an `UNKNOWN_COMMAND` miss retries the final token as a flat topic ID. A visible parent with no matching topic returns `BSC_STATUS_UNKNOWN_TOPIC`. Without a catalog, all tokens after `help` remain one ordinary exact path.
...
The current core implements static descriptor validation, nested path matching, typed positional arguments, selected-command dispatch, output-neutral complete-line console orchestration with caller-owned workspace storage, the pure generated-help layer for metadata validation, exact descriptor-path lookup, deterministic help/manpage rendering, and optional complete-line `help`/`commands` routing through `bsc_execute_line_with_builtins()`. Existing `bsc_execute_line()` remains application-only. Catalog-aware extended help rendering and pure topic pages are implemented; completion, history, authentication, persistent settings, interactive prompts, examples, and adapters remain future staged work until each item has an approved bounded design and host-test plan.

## Generated help staging

The implemented help foundation includes a pure platform-independent renderer: it validates help prose and emitted identifiers separately from ordinary registry validation, performs exact metadata-path lookup, and renders NAME, SYNOPSIS, DESCRIPTION, ARGUMENTS, VALID VALUES, COMMANDS, NOTES, WARNINGS, EXAMPLES, TOPICS, and RELATED sections through `bsc_output_t` depending on the selected ordinary or catalog-aware API. Topic pages render NAME, DESCRIPTION, NOTES, WARNINGS, EXAMPLES, and RELATED, omit SYNOPSIS, and use static examples without prompt markers. The optional complete-line built-ins compose ordinary or catalog-aware renderers through `bsc_execute_line_with_builtins()`. Exact visible paths precede final-token flat-topic fallback, and all help remains isolated from execution callbacks.
