...
When an optional exact-identity catalog is configured, `help <path>` first tries
the complete visible descriptor path and only an `UNKNOWN_COMMAND` miss retries
the final token as a flat topic identifier. The pure APIs remain independently
available: `bsc_help_find_topic()` resolves topics,
`bsc_help_render_catalog_path()` renders catalog-aware command/group pages, and
`bsc_help_render_topic()` renders topic pages. Visibility filtering is
presentation policy and never invokes execution access callbacks.
...
The current core implements static descriptor validation, nested path matching, typed positional arguments, selected-command dispatch, output-neutral complete-line console orchestration with caller-owned workspace storage, the pure generated-help layer for metadata validation, exact descriptor-path lookup, deterministic help/manpage rendering, and optional catalog-aware complete-line `help`/`commands` routing through `bsc_execute_line_with_builtins()`. Existing `bsc_execute_line()` remains application-only. Catalog-aware extended help rendering, pure topic pages, and final-token console topic fallback are implemented; completion, history, authentication, persistent settings, interactive prompts, examples, and adapters remain future staged work until each item has an approved bounded design and host-test plan.

## Generated help staging

The implemented help foundation includes a pure platform-independent renderer: it validates help prose and emitted identifiers separately from ordinary registry validation, performs exact metadata-path lookup, and renders NAME, SYNOPSIS, DESCRIPTION, ARGUMENTS, VALID VALUES, COMMANDS, NOTES, WARNINGS, EXAMPLES, TOPICS, and RELATED sections through `bsc_output_t` depending on the selected ordinary or catalog-aware API. Topic pages render NAME, DESCRIPTION, NOTES, WARNINGS, EXAMPLES, and RELATED, omit SYNOPSIS, and use static examples without prompt markers. The optional complete-line built-ins compose those pure APIs through `bsc_execute_line_with_builtins()`; configured catalogs enable catalog-aware path rendering and final-token flat-topic fallback without creating execution edges.
