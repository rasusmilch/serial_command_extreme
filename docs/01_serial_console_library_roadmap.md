...
- Task 11C-3 catalog-aware console integration: implemented. Console configuration optionally borrows an exact-registry-identity catalog; `bsc_execute_line_with_builtins()` uses catalog-aware path rendering and final-token flat-topic fallback while `bsc_execute_line()` remains application-only.
