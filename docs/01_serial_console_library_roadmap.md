...
### Task 11C — Extended metadata and subtopics: implemented
...
- Task 11C-3 catalog-aware console integration: implemented. Console initialization accepts an optional borrowed catalog with exact registry identity, validates it before activation, and routes exact visible catalog paths before final-token flat-topic fallback; `bsc_execute_line()` remains application-only.
