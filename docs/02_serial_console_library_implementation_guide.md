...
Task 11C-1 adds an optional borrowed `bsc_help_catalog_t` in `src/bsc_help.h` for structural validation of extended help metadata. The catalog wraps one authoritative command table and stores notes, warnings, presentation examples, related descriptor pointers, and flat single-token topic records outside `bsc_command_t`. Catalog validation is structural only and calls registry validation rather than visibility-dependent help validation. Task 11C-2 adds `bsc_help_find_topic()` for pure flat-topic lookup through the catalog authority plus `bsc_help_render_catalog_path()` and `bsc_help_render_topic()` for catalog-aware command/group pages and pure topic pages. Rendering validates metadata and lookup before output, preserves no-metadata byte compatibility with ordinary help, emits NOTES, WARNINGS, EXAMPLES, TOPICS, and RELATED in approved order, and Task 11C-3 integrates the optional borrowed catalog through exact registry identity validation and deterministic full-path-first/final-token-topic console routing.
...
Without a configured catalog, every token after `help` is one ordinary exact descriptor path. With a catalog, routing tries the complete visible descriptor path first. Only when that lookup returns `BSC_STATUS_UNKNOWN_COMMAND`, and at least two tokens follow `help`, routing retries the final token as a flat topic ID beneath the preceding exact parent path. A successful fallback is reported as `BSC_CONSOLE_BUILTIN_HELP_TOPIC`; a visible parent without the requested topic preserves `BSC_STATUS_UNKNOWN_TOPIC`, while a missing or filtered parent remains `BSC_STATUS_UNKNOWN_COMMAND`.
