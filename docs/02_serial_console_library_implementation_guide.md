...
src/
  ...
  bsc_help_extended.c                         # Task 11C-2 topic lookup and catalog-aware rendering
...
Task 11C-1 adds an optional borrowed `bsc_help_catalog_t` in `src/bsc_help.h` for structural validation of extended help metadata. The catalog wraps one authoritative command table and stores notes, warnings, presentation examples, related descriptor pointers, and flat single-token topic records outside `bsc_command_t`. Catalog validation is structural only and calls registry validation rather than visibility-dependent help validation. Task 11C-2 adds `bsc_help_find_topic()` for pure flat-topic lookup through the catalog authority plus `bsc_help_render_catalog_path()` and `bsc_help_render_topic()` for catalog-aware command/group pages and pure topic pages. Task 11C-3 optionally binds that catalog to a console using exact command pointer/count identity. The built-in-aware route tries the complete visible catalog path first and retries only an `UNKNOWN_COMMAND` miss by treating the final token as a flat topic id; ordinary no-catalog operation remains supported.
...
`bsc_console_t` owns no line buffer, token array, parsed-argument array, matcher result, diagnostic storage, mutex, or execution-active state. It borrows descriptor metadata, application context, and the optional immutable help catalog for its lifetime. Catalog configuration requires exact command-table pointer/count identity and structural validation during initialization. When configured, the console stores a by-value copy of the output wrapper while the callback and user pointer remain borrowed.
...
The ordinary pure generated-help renderer remains unchanged. The catalog-aware renderer emits NOTES, WARNINGS, EXAMPLES, TOPICS, and RELATED from validated static metadata; examples have no shell prompt marker and are never parsed, matched, dispatched, executed, or inspected for runtime secret values.
