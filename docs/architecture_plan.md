...
- Generated help/manpages: pure metadata validation, exact descriptor-path lookup, index rendering, command-list rendering, group pages, executable-command pages, pure flat-topic lookup, catalog-aware command/group rendering, pure topic-page rendering, and optional catalog-aware complete-line `help`/`commands` routing are implemented in `src/bsc_help.*`, `src/bsc_help_extended.c`, `src/internal/bsc_help_internal.*`, and `src/bsc_console.*`.
- Extended-help catalog integration: Tasks 11C-1 through 11C-3 are implemented. Console configuration may borrow an exact-registry-identity catalog; built-in help tries a complete visible descriptor path first and then performs final-token flat-topic fallback only after `UNKNOWN_COMMAND`.
...
Task 11B2 optional console built-ins are implemented through the separate `bsc_execute_line_with_builtins()` composition boundary. Tasks 11C-1 and 11C-2 provide the catalog and pure renderers; Task 11C-3 integrates an optional borrowed catalog into the same built-in-aware API without changing application-only `bsc_execute_line()`.
...
Implemented in `src/bsc_help_extended.c` and `src/internal/bsc_help_internal.*`, with the minimal public `bsc_help_topic_lookup_result_t`, `bsc_help_topic_lookup_result_clear()`, `bsc_help_find_topic()`, append-only `BSC_STATUS_UNKNOWN_TOPIC`, `bsc_help_render_catalog_path()`, and `bsc_help_render_topic()`. Existing ordinary help renderers remain byte-compatible and no-metadata catalog-aware rendering is byte-identical to ordinary path rendering.

### Task 11C-3 — Catalog-aware console integration: implemented

Implemented in `src/bsc_console.*`. `bsc_console_config_t` optionally borrows a structurally validated catalog whose command pointer and count exactly match the console registry. `bsc_execute_line_with_builtins()` uses catalog-aware path output when configured, gives a complete visible descriptor path precedence, and retries the final token as a flat topic only after `BSC_STATUS_UNKNOWN_COMMAND`. Built-in routes never invoke application matching, parsing, dispatch, handlers, or execution access callbacks.
...
Generated-help work has byte-exact LF golden-output tests for ordinary and extended pure renderers. Console integration tests compare catalog path/topic built-in output byte-for-byte with those pure renderers and cover full-path precedence, topic status, visibility, output failures, cleanup, and callback isolation. Task 11C-1 catalog validation remains covered by focused host tests for pointer/count policy, descriptor-pointer identity, text/example/topic/related bounds, visibility independence, capacity overrides, and callback non-invocation.

## Current non-goals

The current MVP implements pure generated help/manpages and optional catalog-aware complete-line `help`/`commands` routing, but does not implement examples, adapters, history, completion, line editing, aliases, optional positional arguments, runtime registration, authentication, platform locks, automatic diagnostics, automatic final-result output, packaging, license selection, or CI workflows.
