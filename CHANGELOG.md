...
- Extended-help catalog foundation with borrowed notes, warnings, presentation examples, related descriptor references, flat topic metadata, structural validation diagnostics, pure flat-topic lookup, topic-specific `BSC_STATUS_UNKNOWN_TOPIC`, catalog-aware command/group rendering, pure topic-page rendering, byte-exact extended golden fixtures, exhaustive output-failure coverage, configurable-capacity fixture coverage, and no-metadata compatibility.
- Optional borrowed catalog configuration for consoles, with exact registry pointer/count identity and structural validation during initialization; catalog-aware built-in help renders exact visible paths before final-token flat-topic fallback, reports a distinct topic route, preserves `UNKNOWN_TOPIC` versus `UNKNOWN_COMMAND`, and remains isolated from application execution callbacks without heap or full-page buffering.
...
- Active documentation now reflects the pure help foundation, optional catalog-aware console built-ins, the descriptor/help model, separate help metadata validation, static visibility policy, descriptor-order LF output, extended sections, flat topic fallback, and the historical status of the original handoff.
