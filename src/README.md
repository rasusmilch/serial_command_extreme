...
Optional catalog-aware console built-in routing is implemented in `bsc_console.*`; planned future modules still include adapters and examples. Exact future names may change, but the core constraints must remain intact.
...
Every source-code task must include host tests for new or changed parser, tokenizer, registry, validation, matcher, dispatch, output, redaction, help, or access behavior. The typed positional parser, compact-float diagnostics, selected-command access enforcement, dispatch, catalog validation, pure flat-topic lookup, catalog-aware command/group rendering, pure topic-page rendering, and catalog-aware console routing are implemented; adapters and examples remain future work.
