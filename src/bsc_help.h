 ...
 * metadata never affects execution; optional console routing composes only the
 * pure lookup/rendering APIs and does not make topics executable.
 */
 ...
 * #bsc_help_render_topic. Console built-ins may route a final token to this pure
 * renderer after an exact full-path miss. Task 11C adds no nested topics, topic
 * visibility flags, topic access levels, or topic-to-topic relationships.
 * Static examples are presentation text only and should use application-authored
 * placeholders such as <new-password>, ********, or <secret> for secret arguments.
 *
